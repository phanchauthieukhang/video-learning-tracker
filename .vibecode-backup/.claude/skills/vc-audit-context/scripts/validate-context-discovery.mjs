...
if (!bareKitMode) {
  ...
  for (const doc of contextDocs) {
    ...
    if (normalizedDoc === router) continue;
    const relFromContext = normalizedDoc.replace(/^process\/context\//, "");
    const group = relFromContext.split("/")[0];
    const groupEntrypoint = getGroupEntrypoint(group);
    const indexedByRouter = routerText.includes(relFromContext) || routerText.includes(normalizedDoc);
    const indexedByGroup = groupEntrypoint && read(groupEntrypoint).includes(path.basename(normalizedDoc));
    ...
    if (!indexedByRouter && !indexedByGroup) {
      fail(`${normalizedDoc} is not indexed by process/context/all-context.md or its group entrypoint`);
    }
  }
